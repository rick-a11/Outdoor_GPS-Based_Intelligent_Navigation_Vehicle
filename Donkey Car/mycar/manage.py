...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    gps_latest_position = V.get_part_by_name('GpsLatestPosition')
    ...
    g = get(path,origin_reset,ctr,gps_latest_position)
    g.start_polling()
    V.add(g,threaded=True,inputs=[],outputs=['signal'])
    print("get  success")
    ...
    def switch_to_new_path():
        new_path_filename = "onenet.csv"  # 假设你从 OneNet 获取的路径点保存到这个文件
        if os.path.exists('onenet.csv') and path.load('onenet.csv'):
           print("The path was loaded was loaded from 'onenet.csv" )
   ...
   # This will use path and current position to output cross track error
    cte = CTE(ctr,g,look_ahead=cfg.PATH_LOOK_AHEAD, look_behind=cfg.PATH_LOOK_BEHIND, num_pts=cfg.PATH_SEARCH_LENGTH)
