...
class SerialPort:
    ...
    #计算数据校验位并拼接在一起发送
    def Send_Command(self, ser, data):
    ...
    #初始化并退出备份模式，使 GPS 模块正常工作
    def Serial_Init(self):
        logger.info("正在初始化GPS模块")

        # 设置GPIO引脚模式为BCM编号方式
        GPIO.setmode(GPIO.BCM)
        # 关闭GPIO库的警告
        GPIO.setwarnings(False)
        # 再次设置GPIO引脚模式为BCM编号方式
        GPIO.setmode(GPIO.BCM)
        # 配置FORCE引脚为输入模式
        GPIO.setup(self.FORCE, GPIO.IN)
        # 配置STANDBY引脚为输出模式，并设置为高电平
        GPIO.setup(self.STANDBY, GPIO.OUT)
        GPIO.output(self.STANDBY, GPIO.HIGH)

        # 设置波特率
        self.ser = serial.Serial(self.port, 9600, self.bits, self.parity, self.stop_bits, timeout=self.timeout)
        self.Send_Command(self.ser, "$PMTK251,115200")
        time.sleep(2)
        self.ser = serial.Serial(self.port, 115200, self.bits, self.parity, self.stop_bits, timeout=self.timeout)

        # 设置GPIO口以启动GPS模块
        GPIO.setup(self.FORCE, GPIO.OUT)
        time.sleep(1)
        GPIO.output(self.FORCE, GPIO.HIGH)
        time.sleep(1)
        GPIO.output(self.FORCE, GPIO.LOW)
        time.sleep(1)
        GPIO.setup(self.FORCE, GPIO.IN)

        logger.info("GPS模块初始化完成")
